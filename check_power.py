...
def check_power(battery_1, battery_2, no_battery_1_series, no_battery_1_parallel, no_battery_2_series, no_battery_2_parallel, original_max_mass, max_volume, required_power, req_charging_power):
    ...
    if peak_charge_power_generated < peak_power_generated:
        if peak_power_generated < required_power:
            return original_max_mass, max_volume
        
        else: 
            max_mass = original_max_mass - (math.ceil((peak_power_generated)/62000))
            # print(desired_EV_characteristics[2])
            max_volume = max_volume - (math.ceil((peak_power_generated)/143000000))
        ...
        if peak_charge_power_generated < required_power:
            ...
            max_mass = original_max_mass - (math.ceil((peak_charge_power_generated)/62000))
            # print(desired_EV_characteristics[2])
            max_volume = max_volume - (math.ceil((peak_charge_power_generated)/143000000))
            return max_mass, max_volume
